  ...
  _createClass(Linter, [{
    ...
    value: function grepDirs(pathnames) {
      ...
      return Promise.all(pathnames.map(function (pathname) {
        ...
        return Promise.reject('Linter#grepDirs error ' + err);
      });
    }
    ...
    */
    ...
    value: function grepDir(pathname) {
      ...
      if (!grepExpression) {
        config = new _config2.default(pathname, this.options).load();

        _bragi2.default.log('Linter:grepDir:debug', 'looking config file', config);
        var rules = config.rules.map(function (rawRule) {
          return new _rule2.default(rawRule);
        }).map(function (rule) {
          return rule.value.join('|');
        });

        _bragi2.default.log('Linter:grepDir:debug', 'rules', rules);
